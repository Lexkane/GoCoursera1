...
func main() {
	...
	node, err := parser.ParseFile(fset, osArgs[1], nil,
...
SPECS_LOOP:
	for _, spec := range g.Specs {
		...
		for _, comment := range g.Doc.List {
			needCodegen = needCodegen || strings.HasPrefix
			(comment.Text, "cgen:binpack" )
		}
		...
		FIELDS_LOOP:
			for _,field:=range currStruct.Fields.List{
				...
				if field.Tag !=nil{
					tag:=reflect.StructTag(field.Tag.Value[1:
						len(field.Tag.Value)-1])
						if tag.Get("cgen")=="-"{
			...
			fmt.Printf("\tgenerating code for field %s.%s\n",
				...
				switch fileType{
					...
					default:
						log.Fataln("unsupported",fileType)	
				}
...
//go build gen/* && ./codegen.exe pack/unpack.go
//pack marshaller.go
//go run pack/*
